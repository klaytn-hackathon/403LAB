...
////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  changeUI: async function (walletInstance) {
    ...
    // $('#address').append('<br>' + '<p>' + '내 계정 주소: ' + walletInstance.address + '</p>'); //내 계정 정보 표시
    // //html의 #address 아이디를 지닌 곳에 뒤의 메시지를 append(추가)시켜 준다. //추가 전에는 비워두고 추가 이후 append를 통해 표시    

    // //HTML UI에 컨트랙의 잔액을 추가해준다.
    // $('#contractBalance')
    // .append('<p>' + '이벤트 잔액: ' + cav.utils.fromPeb(await this.callContractBalance(), "KLAY") + '</p>'); //잔액을 가져올때 PEB로 불러오니 KLAY로 변환해 메시지 보낸다.    

    // //송금하는 UI의 경우 관리자만 볼 수 있게 변환
    // if (await this.callOwner() === walletInstance.address) {
    //   $('#owner').show(); //관리자인 경우에만 html의 관리자 UI 변경
    // }
  },
  ...
  // //node를 통한 회원 가입 여부 확인
  // isJoin: async function () {
  //   return await agContract.methods.students().call();
  // }    
  ...
  //임시 그룹 리스트 간단 목록 받아오기
  //모집 공고 등록 버튼
  tempGroupSimpleList: function () {    
    var URL = 'http://mijuhome.iptime.org:5333/tempGroupSimpleList';
    $.ajax({
      url: URL,
      type: 'get',
      success: function(data) {
        ...
        { 
          var st = recvData[i].studySt.split("T")[0];
          var ed = recvData[i].studyEd.split("T")[0];

            $('#simpleGroupIntro').append(
              '<div class="col-md-4">\
                <div class="card mb-4 shadow-sm ">\
                  <div class="card-body">\
                    ...
                    <div class="d-flex justify-content-between align-items-center">  \
                      <small class="text-muted">모집 기간<br/>' + st + ' ~ ' + ed + '</small>\
  ...
  joinGroup: function (num) {
    var groupIndex = num;
    var walletInstance = this.getWallet();
    var address = walletInstance.address;

    // console.log(groupIndex); //ajax를 통해 주소 보내고 DB에 추가 시켜야함

    var sendArray = {};
    sendArray['userAddress'] = address;
    sendArray['groupid'] = num;

    //평점 추가해서 거르기

    var url = 'http://mijuhome.iptime.org:5333/joinGroup';
